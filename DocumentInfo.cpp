...
bool DocumentInfo::PasteFromClipBoard( int iMode, ViewInfo* pAlternativeViewInfo )
{
  ...
  ||        ( dwTargetDepth == 15 ) )
  {
    ...
    {
      AfxMessageBox( GR::Convert::ToUTF16( CMisc::printf( "Nicht unterst¸tzte Umwandlung von %d auf %d!", pPack->m_pImage->GetDepth(), dwTargetDepth ) ).c_str() );
      SafeDelete( pPack->m_pImage );
      SafeDelete( pPack );
      return false;
    }
  ...
  {
    AfxMessageBox( GR::Convert::ToUTF16( CMisc::printf( "Nicht unterst¸tzte Farbtiefe %d!", pPack->m_pImage->GetDepth() ) ).c_str() );
    SafeDelete( pPack->m_pImage );
    SafeDelete( pPack );
    return false;
  }
...
void DocumentInfo::MapToPalette( GR::Graphic::Palette *pNewPalette )
{
  ...
  {
    theApp.m_pMainWnd->MessageBox( _T( "Nicht unterst¸tztes Format (nur Einzelbilder im Moment)." ), _T( "Hinweis" ), 0 );
    return;
  }
